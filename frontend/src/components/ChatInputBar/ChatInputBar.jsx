...
import useImageUpload from "../../hooks/useDropzone";

const ChatInputBar = ({ onSendMessage, onSendCandleData }) => {
  ...
  const handleInternalCandleData = useCallback((candleBlock) => {
    setInputMessage((prev) => `${prev}${candleBlock}`);
  }, []);

  useEffect(() => {
    if (onSendCandleData) {
      onSendCandleData(handleInternalCandleData);
    }
  }, [onSendCandleData, handleInternalCandleData]);
